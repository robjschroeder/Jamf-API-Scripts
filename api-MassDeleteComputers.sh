...
file="/Users/AdminUser/Desktop/ids.txt"
...
checkVariables(){
	...
	# File
	if [ $file = "" ]; then
		read -p "Please drag your file into terminal: " file
	fi
}
...
deleteComputers(){
	# Read in the IDs from the file
	ids+=($(cat $file))
	# Loop through
	for id in "${ids[@]}"; do
		echo "Delete computer: $id"
		curl -X DELETE "${jamfProURL}/api/v1/computers-inventory/$id" -H "accept: */*" -H "Authorization: Bearer $token"
	done
}
