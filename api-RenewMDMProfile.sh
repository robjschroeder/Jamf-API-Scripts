...
# Renews the device's MDM Profile, including the device 
# identity certificate within the MDM Profile.
#
...
# Array of UUIDs of Devices that need a renew MDM profile command
UDIDs=$(cat <<EOF
...
renewMDM(){
	echo "Renew"
	curl --request POST \
	--url ${jamfProURL}/api/v1/mdm/renew-profile \
